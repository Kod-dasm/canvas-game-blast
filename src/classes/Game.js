...
export default class Game {
  constructor (_sizeStar = 48, _countStarX = 10, _countStarY = 10, _speed = 96, _radiusBomb = 3, _minCountStarsBurn = 2, _moves = 100) {
    this.sizeStar = _sizeStar
    this.countStarX = _countStarX
    this.countStarY = _countStarY
    this.speed = _speed
    this.radiusBomb = _radiusBomb
    this.minCountStarsBurn = 2
    this.bonuse = 100
    this.moves = _moves
    this.field = []
  ...
  addCell(columnNumber) {
    ...
    for (let rowNumber = 0; rowNumber < this.countStarY; rowNumber++) {
      cells.push(this.createStar(10 + columnNumber * this.sizeStar, 10 + (this.countStarY - (rowNumber + 1)) * this.sizeStar))
    }
    return cells
  }
  createStar (columnNumber,rowNumber) {
    const starTemplate = this.stars[STAR_COLORS[Math.floor(Math.random() * STAR_COLORS.length)]]
  ...
  addStar(columnNumber, rowNumber) {
    this.field[columnNumber].push(this.createStar(10 + columnNumber * this.sizeStar, 10 + (this.countStarY - (rowNumber + 1)) * this.sizeStar))
  }
  updateCoordStar(columnNumber, rowNumber) {
    this.field[columnNumber][rowNumber].x = 10 + columnNumber * this.sizeStar
    this.field[columnNumber][rowNumber].y = 10 + (this.countStarY - (rowNumber + 1)) * this.sizeStar
  }
  animationOfShootingStars() {
    const game = this
    for (let rowNumber = 0; rowNumber < game.countStarY; rowNumber++) {
    let step = 0;
    // game.animation(rowNumber, step)
    (function animation(){
...
// animation(rowNumber, step) {
//   if (this.field.length != 0) {
//     this.moveStars(rowNumber + 1)
//     this.drawField(rowNumber, step + 1)
//     step++
//     if (step < this.speed) {
//           console.log(this.field.length != 0)
//           window.requestAnimationFrame(this.animation(rowNumber, step))
//         }
//       }
//   }
  moveStars(currentRowNumber) {
    for (let rowNumber = 0; rowNumber < currentRowNumber; rowNumber++) {
      for (let columnNumber = 0; columnNumber < this.countStarX; columnNumber++) {
        if (this.field[columnNumber][rowNumber].currentPlace < this.field[columnNumber][rowNumber].y) {
          this.field[columnNumber][rowNumber].currentPlace += this.sizeStar / this.speed
        }
      }
    }
  }
  // function drawField(currentRowNumber = this.countStarY, step = 1) {
  ...
  drawBackground() {
    ...
    if (!background.loaded){
      background.img.addEventListener("load", this.drawImageLoaded(background, 0, 0, this.sizeFieldX, this.sizeFieldY), false);
    }
    else {
      this.ctx.drawImage(this.img.fon.img, 0, 0, this.sizeFieldX, this.sizeFieldY);
    }
  }
  ...
  drawStar(columnNumber, rowNumber, sizeStar = this.sizeStar) {
    ...
    // const color = colorStar(.color)
    // const stepAnimation = step * this.sizeStar / this.speed
    if (!star.template.loaded){
      star.template.img.addEventListener("load", this.drawImageLoaded(star.template, star.x, star.currentPlace, sizeStar, sizeStar), false);
    }
    else {
      this.ctx.drawImage(star.template.img, star.x, star.currentPlace, sizeStar, sizeStar)
    }
  }
}